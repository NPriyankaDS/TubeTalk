...
# Defining the genai as the client
def google_client(api_key):
...
def main():
    ...
    st.markdown("<div style='text-align: center; font-size: 34px; font-weight: bold; color: blue;'> Welcome to TubeTalk </div>", unsafe_allow_html=True)
    st.markdown("<div style='text-align: center; font-size: 22px; color:pink;'>\"Turning Monologues into Conversations\"</div>", unsafe_allow_html=True)
    st.divider()
    ...
    with st.sidebar:
        st.markdown("### About Tubetalk")
        st.markdown("""
        TubeTalk transforms YouTube videos into interactive conversations. 
        Simply paste a URL and chat with your video content to get summaries and answer questions.
        ...
        if google_api_key:
            client = google_client(api_key=google_api_key)
            st.session_state["api_key"] = google_api_key
            try:
                # Initialize the client and store it in session state
                st.session_state["genai_client"] = google_client(st.session_state["api_key"])
                st.success("API Key validated and client initialized!")
            except Exception as e:
                st.error(f"Error initializing client: {e}")
        
        youtube_url = st.text_input("Enter a valid Youtube URL", help="A valid Youtube URL eg: https://www.youtube.com/watch?v=rAeqTaYj_aI")
    ...
    if "genai_client" in st.session_state:
        if youtube_url:
            ...
            if transcript_with_timestamps_str and not st.session_state.chatbot_initialized:
                ...
                if chat_button:
                    with st.spinner("Initializing TubeTalk"):
                        # Initialize the QA Chatbot
                        st.session_state.chatbot = QAChatbot(google_api_key=google_api_key, db_name="qa_database")
                        
                        # Add documents to the database
                        documents = [transcript_with_timestamps_str]
                        st.session_state.chatbot.add_documents(documents)
                        st.session_state.chatbot_initialized = True
                        st.success("TubeTalk initialized successfully!")

            # Display chat interface only when chatbot is initialized
            if st.session_state.chatbot_initialized:
                # Display welcome message only once
                if not st.session_state.conversation_history:
                    welcome_message = "Hello! Welcome to Tubetalk, I am your QA assistant. Ask me any questions related to the Youtube video."
